...
class CineHuntSdk{
	...
	apiUrl: string; 
	constructor(apiUrl: string = baseApiUrl){
		this.apiUrl = apiUrl;
	}

	convertMovieDetails(movieDetailsData: MovieDetailsResponse): MovieDescriptionDTO {
		// take the in
	...
	async requestMoviesFromAPI(paramKey: string, query: string, apiUrl: string): Promise<any>{
	...
	async getMovieByName(movieName: string): Promise<MovieDTO[]> {
		console.log(`Requested Movie Title: ${movieName}`);
		// trim any trailing or leading whitespace from the movie name 
		movieName = movieName.trim();
		
		let movieList: MovieSearchResponse = await this.requestMoviesFromAPI('q', movieName, baseApiUrl);
	...
	async getMovieById(imdbId: string): Promise<MovieDescriptionDTO>{
		imdbId = imdbId.trim();
		console.log(`Requested Movie Id: ${imdbId}`);
		const movieData: MovieDetailsResponse = await this.requestMoviesFromAPI('tt', imdbId, baseApiUrl);
		const movieDetails = this.convertMovieDetails(movieData); 
		return movieDetails;
	
	}
	// generate random movie API fetcher
	async getTenRandomMovies(): Promise<MovieDTO[]> {
...
export { CineHuntSdk, MovieDTO, MovieDescriptionDTO };